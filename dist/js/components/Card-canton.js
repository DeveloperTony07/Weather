app.component('card-canton', {
    ...
    mounted() {
        this.cordinate1 = this.latitute;
        this.cordinate2 = this.longitude;
        axios({
            method: 'get',
            url: 'https://api.weatherapi.com/v1/current.json?key=df53bb4e93f6408084e211804231405&q=' + this.latitute+ ','+this.longitude +' &aqi=no'
        })
            .then(
                (response) => {
                    let item = response.data;
                    console.log(item.location.region);
                    
                    this.canton = item.location.name;
                    if (this.canton === "Esparta"){
                        this.canton = "Esparza"
                    }else if (this.canton === "Canas"){
                        this.canton = "Cañas"
                    }else if (this.canton === "Daniel Flores"){
                        this.canton = "Perez Zeledón"
                    }

                    this.province = item.location.region
    ...
    <div :class="clr">
    <div class="row-1">
        <h2 class="txt-name-canton">{{canton}}<br>
            <span> {{ province }}</span>
        </h2>
    ...
    <p class="txt-cloudines">{{ cloudiness }}</p>
    <div class="row">
